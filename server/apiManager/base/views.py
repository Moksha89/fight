# views.py — base API views with Redis caching for read-heavy endpoints
...
from kokoroko.cache_helpers import (
    get_cached_or_set, KEYS, TTL,
)
...
class SettingViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        def _fetch():
            data = list(SettingSerializer(Setting.objects.all(), many=True).data)
            data.append({
                "id": 0,
                "action": "#",
        ...
        data = get_cached_or_set(KEYS["settings"], _fetch, TTL["settings"])
...
class StatusViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        def _fetch():
            return list(StatusSerializer(
                Status.objects.filter(isActive=True), many=True
            ).data)

        data = get_cached_or_set(KEYS["statuses"], _fetch, TTL["statuses"])
        return Response(data)
...
class BannerViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        def _fetch():
            return list(BannerSerializer(Banner.objects.all(), many=True).data)

        data = get_cached_or_set(KEYS["banners"], _fetch, TTL["banners"])
        return Response(data)
...
class HighlightViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        def _fetch():
            return list(HighlightSerializer(
                Highlight.objects.all(), many=True
            ).data)

        data = get_cached_or_set(KEYS["highlights"], _fetch, TTL["highlights"])
        return Response(data)
