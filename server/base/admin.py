...
class SettingAdmin(admin.ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        # Invalidate caches on setting change
        try:
            from kokoroko.cache_helpers import invalidate_settings
            invalidate_settings()
            if obj.action == 'Z':
                from django.core.cache import cache
                cache.delete('active_theme')
                cache.delete('feature_controls')
        except Exception:
            pass
...
class StatusAdmin(MediaDeleteMixin, MediaUpdateMixin, admin.ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        super().save_model(request, obj, form, change)
        try:
            from kokoroko.cache_helpers import invalidate_statuses
            invalidate_statuses()
        except Exception:
            pass
...
class BannerAdmin(MediaDeleteMixin, MediaUpdateMixin, admin.ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        super().save_model(request, obj, form, change)
        try:
            from kokoroko.cache_helpers import invalidate_banners
            invalidate_banners()
        except Exception:
            pass
...
class HighlightAdmin(MediaDeleteMixin, MediaUpdateMixin, admin.ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        super().save_model(request, obj, form, change)
        try:
            from kokoroko.cache_helpers import invalidate_highlights
