...
def get_odds_config():
    """Get odds config from cache, falling back to DB."""
    ...
    return get_cached_or_set(KEYS["odds_config"], _fetch, TTL["odds_config"])


def _get_odds_config_fresh():
    """Get odds config directly from DB (for writes/recalculations)."""
    from cockfightManager.models import OddsConfig
    config, _ = OddsConfig.objects.get_or_create(id=1)
    return config
...
def recalculate_dynamic_odds(config=None):
    ...
    from kokoroko.cache_helpers import invalidate_odds
    invalidate_odds()
...
def recalculate_rebalance_odds(config=None):
    ...
    # Invalidate cache after rebalance
    from kokoroko.cache_helpers import invalidate_odds
    invalidate_odds()
