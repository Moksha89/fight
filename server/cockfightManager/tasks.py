...
def _cancel_unprocessed_matches():
    """Cancel all unprocessed matches using .save() so bet refunds trigger."""
    orphaned = CockfightAutoMatch.objects.filter(processed=False, winTeam=0)
    for m in orphaned:
        if m.recordingStatus == 'recording':
            try:
                stop_match_recording(m)
            except Exception:
                pass
        m.winTeam = 4
        m.save()  # triggers process_cockfight_match_result via model save()
        logger.info(f"Cancelled orphaned match {m.matchNumber} (id={m.pk})")
...
def _poll_auto_match_inner(state):
    ...
    if not state.runningMatchRefId:
        # Recover missed results from history API before cancelling
        _recover_missed_results()
        _cancel_unprocessed_matches()
...
def check_stream_health():
    """
    Check if the cockfight live stream (HLS) is accessible.
    If down: pause betting on current auto match to protect users.
    If back up: resume betting.
    """
    ...
    try:
        state = AutoMatchPollingState.objects.filter(id=1).first()
        if not state or not state.runningAutoMatchId:
            return "No active auto match"

        match = CockfightAutoMatch.objects.filter(pk=state.runningAutoMatchId).first()
        if not match:
            return "Match not found"

        # Try to reach the HLS stream endpoint
        stream_ok = False
        try:
            req = urllib.request.Request(
                'https://origin.theclandestineproject.live/MainArena/streams',
                method='GET'
            )
            resp = urllib.request.urlopen(req, timeout=8)
            if resp.status == 200:
                data = json.loads(resp.read())
                # Stream is available if we get valid data
                stream_ok = bool(data)
        except Exception:
            stream_ok = False

        # Also check the external cockfight API
        api_ok = False
        try:
            response = requests.get(API_URL, timeout=5)
        ...
        if not stream_ok and not api_ok:
            # Both stream AND API are down - stop accepting bets
            if state.isAcceptingBet:
                state.isAcceptingBet = False
                state.save(update_fields=['isAcceptingBet'])
                logger.warning("Stream & API down - paused betting for auto match")

                # Broadcast the update so frontend knows
                try:
                    channel_layer = get_channel_layer()
                    async_to_sync(channel_layer.group_send)(
                        "match_updates",
                    ...
                    pass
            return "Stream down - betting paused"

        elif api_ok and not state.isAcceptingBet:
            # API is back - the normal poll_auto_match_status will handle resuming
            return "API back up - normal polling will resume betting"

        return f"Stream: {'OK' if stream_ok else 'DOWN'}, API: {'OK' if api_ok else 'DOWN'}"

    except Exception as e:
        return f"Stream health check error: {e}"


def _recover_missed_results():
    """Fetch history from external API and set winners for any unprocessed matches."""
    try:
        resp = requests.get(HISTORY_API_URL, timeout=10)
        resp.raise_for_status()
        data = resp.json()
        results = data.get('resultData', {}).get('list', [])
    except Exception as e:
        logger.error(f"Failed to fetch match history for recovery: {e}")
        return

    history_map = {}
    for r in results:
        ref_id = str(r.get('id', ''))
        win_team = r.get('winTeam', 0)
        if ref_id and win_team:
            history_map[ref_id] = win_team

    unprocessed = CockfightAutoMatch.objects.filter(processed=False, winTeam=0)
    recovered = 0
    for match in unprocessed:
        ref = str(match.referanceId)
        if ref in history_map:
            match.winTeam = history_map[ref]
            match.save()  # triggers process_cockfight_match_result
            recovered += 1
            logger.info(f"Recovered match {match.matchNumber} (id={match.pk}): winner={history_map[ref]}")
    if recovered:
        logger.info(f"Recovered {recovered} missed match results from history API")
