...
async function hydrateSiteConfig() {
  try {
    ...
    const game=siteConfig.featured_game;
    const liveStream=siteConfig.stream||{};
    const current=store.getState().match;
    const match=matchFromGame(game,current,liveStream);
    store.setState({siteConfig,match});
  }
...
function normalizeResult(raw = {}) {
  ...
  return {id:raw.fightNumber||raw.matchNumber||raw.id||'—',winner,tone:winTeam===1?'red':winTeam===2?'blue':'gold',result:winTeam===4?'Cancelled':'Settled',endedAt:raw.result_declared_at||raw.endedAt||raw.created_at||new Date().toISOString()};
}
...
function connectLiveServices() {
  ...
  if(current.authenticated)liveServiceTimers.push(window.setInterval(()=>hydrateAccount(),15000));
}
...
document.addEventListener('click',event=>{
  ...
  else if(action==='select-screen'){const current=store.getState();const game=(current.siteConfig?.games||[]).find(item=>String(item.id)===String(control.dataset.gameId));store.setState({selectedScreen:Number(control.dataset.screen),match:matchFromGame(game,current.match),quote:null,selectedOutcome:null});showToast(`${game?.title||`Match ${control.dataset.screen}`} selected.`,'success');}
